...
def test_interrupt_does_not_wait_for_lazy_agent_initialization(monkeypatch):
    """A newly created session can be stopped before agent_ready is set."""
    ...
    # _session() treats agent=None as "use a placeholder", so set the real lazy
    ...
    try:
        monkeypatch.setattr(
            ...
            "_start_agent_build",
            lambda *_args, **_kwargs: (_ for _ in ()).throw(
                AssertionError("interrupt must not start or wait for agent initialization")
            ),
        )
