...
def _(rid, params: dict) -> dict:
    # Stop must remain available while the lazy agent is still building. Using
    # _sess() here waits up to 30 seconds for agent_ready and can make an
    # immediate post-create Stop time out even though cancelling only needs the
    # session's local turn state.
    session, err = _sess_nowait(params, rid)
    if err:
        return err
